...
class Auth_model extends BaseMySQL_model
{
	...
	public function login($data)
	{
		...
		if ($user && $user['status'] == STATUS_ACTIVE && $this->User->verifyPassword($password, $user['password']))
		{
			// Auto-rehash: if password is still MD5, upgrade to bcrypt
			if ($this->User->needsRehash($user['password'])) {
				$newHash = $this->User->hashPassword($password);
				$this->User->update($user['id'], array('password' => $newHash));
			}
			unset($user['password']);
			return $user;
		}
	...
	public function setRandomPassword($username)
	{
		...
		$res = $this->user->update_user($username, array('password' => $this->User->hashPassword($random_password)));
