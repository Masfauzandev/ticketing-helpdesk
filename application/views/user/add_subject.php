            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h3 class="h4">Add Subject</h3>
                    </div>
                    <div class="card-body">
                        <div class="form-group">
                            <input type="hidden" id="edit_id">
                            <label>Subject Name</label><code>*</code>
                            <input type="text" id="name" class="form-control" placeholder="Enter Subject Name">
                        </div>
                        <div class="form-group">
                            <button id="btn_save" class="btn btn-primary">Save</button>
                            <a href="<?= BASE_URL ?>user/add_subject" class="btn btn-secondary">Cancel/Reset</a>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                    ...
                                    <th style="font-size: 1.2rem;">Subject Name</th>
                                    <th style="font-size: 1.2rem;">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php if (!empty($list_subject)): ?>
                                    <?php foreach ($list_subject as $c): ?>
                                        <tr>
...
<script>
    $(document).ready(function () {
        $("#btn_save").click(function () {
            ...
            var url = BASE_URL + 'API/Master/add_subject';
            var data = { 'name': nama };

            if (edit_id) {
                url = BASE_URL + 'API/Master/update_subject';
                data.id = edit_id;
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.result) {
                        var msg = edit_id ? 'Subject updated successfully' : 'Subject added successfully';
        ...
        $(document).on('click', '.delete-item', function () {
            ...
            $.ajax({
                type: 'POST',
                url: BASE_URL + 'API/Master/delete_subject',
                dataType: 'json',
                data: { 'id': id },
                success: function (response) {
                    if (response.result) {
