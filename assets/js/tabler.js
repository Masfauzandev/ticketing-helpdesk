	...
	function _makeAPage(element, path, options) {
		...
		_getPageInformation(path, function (err, data) {
			...
			if (!options.disableFilters) {
				html += '<div class="col-md-12"> <div class="tik-filter-'+rnd+' horizontal-alpaca col-md-12"></div></div><hr><br/>';
			}
			html += '<div class="col-md-12"> <table class="tik-datatable-'+rnd+' table dataTable nowrap" style="width:100%;"></table></div></div>';
			element.html(html);
			...
			// Render filters after callback (non-critical, may fail if alpaca not loaded)
			try {
				if (!options.disableFilters) {
					if (options.serverSide) {
				...
				console.warn('Filter initialization skipped:', e.message);
			}
		});
	}
